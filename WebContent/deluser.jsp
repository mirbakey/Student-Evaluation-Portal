...
<div id="container">
	...
	<%			
		...
		if (validity.equals("truetrue")) {			
			...
			<div id="blocks">
			<h3 class="fie">List Of All Users </h3>
			<h4 class="fie">Select To Delete User</h4>
			
			<form action="deluserfinal.jsp" method="post">
			<table>
			<tr><th>S.No</th><th>Name</th><th>Email</th><th>Contact</th><th>Qualification</th><th>Selection</th></tr>
			<%int i =0;
			while(rs.next())
			{
				...
				<tr><td><%=i%></td><td><%=name %></td><td><%=compEmail%></td><td><%=contact%></td><td><%=qualif%></td><td><input type="radio" value=<%=compEmail%> name="email" required></td></tr>
